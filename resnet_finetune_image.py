...
def rescale(images, scaled_size):
    images = np.squeeze(images, axis=3)
    X = []
    for img in images:
        X.append(scipy.misc.imresize(img, (scaled_size, scaled_size)))
    X = np.array(X)
    X = np.expand_dims(X, axis=3)
    return X


def load_data(filename, scaled_size):
    with h5py.File(data_dir+filename, 'r') as h5f:
        X = h5f['X']
        Y = h5f['Y']

        positives = np.nonzero(Y[:, 1])[0]
        negatives = np.nonzero(Y[:, 0])[0]

        negatives = np.random.choice(negatives, len(positives), replace=False)

        indexes = np.append(positives, negatives)
        np.random.shuffle(indexes)

        X = np.take(X, indexes, axis=0)
        Y = np.take(Y, indexes, axis=0)

        X = rescale(X, scaled_size)
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        train_x_1d, train_y = load_data(train_file, image_size)
        ...
        for batch in batches:
            ...
            if cur_step % evaluate_every == 0:
                val_x_1d, val_y = load_data(val_file, image_size)
