...
model = tflearn.DNN(network, tensorboard_verbose=0, checkpoint_path='./data/checkpoints/nodule-classifier.tfl.ckpt')
model.fit(X_train_images, Y_train_labels, n_epoch = 70, shuffle=True,
			validation_set = (X_val_images, Y_val_labels), show_metric = True,
			batch_size = 100, snapshot_step=15, snapshot_epoch = True, run_id = 'nodule-classifier')
model.save("./data/nodule-classifier.tfl")
